...
def write_to_file(all_weather_data):
    weather_data_dir = "/Users/elliekavanagh/COMP30830/weather_forecast"

    # Fix: Check the correct directory variable
    if not os.path.exists(weather_data_dir):
        os.makedirs(weather_data_dir, exist_ok=True)
        print("Folder 'weather_forecast' created!")
    else:
        print("Folder 'weather_forecast' already exists.")
    
    now = datetime.datetime.now()
    timestamp = now.strftime("%Y-%m-%d %H:%M:%S")
    
    # Fix: Ensure correct file path
    filename = os.path.join(weather_data_dir, f"weather_forecast_{timestamp.replace(' ', '_').replace(':', '-')}.json")
...
def main():
    ...
    try:
        for location in dbinfo.locations:  # Loop through each location
            ...
            Weather_URI = f"https://api.openweathermap.org/data/2.5/forecast?lat={lat}&lon={lng}&appid={dbinfo.WeatherKey}&units=metric"
            
            r = requests.get(Weather_URI)  # Fix: Remove redundant params
